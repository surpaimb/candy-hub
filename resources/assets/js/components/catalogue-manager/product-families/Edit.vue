...
export default {
  ...
  methods: {
    load() {
      apiRequest
        .send('get', '/product-families/' + this.id, [], this.params)
        .then(response => {
          this.family = response.data;
          this.setUp(this.family.attribute_data);
          this.attribute_groups = this.family.attribute_groups.data;
          document.title =
            this.$options.filters.attribute(this.family, 'name') +
            ' Product Family - GetCandy';
    ...
    <div>
        <template v-if="loaded">
            <candy-tabs initial="familyattributes">
                <candy-tab :name="$t('product.Details')" :selected="true" dispatch="product-family-details">
                ...
                <candy-tab :name="$t('product.AttributeGroups')" dispatch="product-family-details">
