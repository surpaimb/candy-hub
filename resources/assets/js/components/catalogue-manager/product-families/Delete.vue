        ...
        <candy-modal title="Confirm deletion" v-show="showModal" size="modal-md" @closed="showModal = false">
            <div slot="body">
                ...
                <div class="alert alert-danger" v-if="!selected">
                    {{$t('product.ChooseProductPrompt')}}
                </div>
                <p>{{$t('product.SelectProductPrompt')}}</p>
                ...
                <template v-else>
                    <p>{{$t('product.RemoveProductPrompt')}}</p>
                </template>
            </div>
            <template slot="footer">
                <button type="button" class="btn btn-primary" @click="trash">{{$t('common.Delete')}} {{ element }}</button>
            </template>
        </candy-modal>
    </div>
</template>
...
export default {
  ...
  methods: {
    trash() {
      this.request
        .send('delete', '/product-families/' + this.id, {
