...
export default {
  ...
  mounted() {
    this.request.send('GET', 'product-families').then(response => {
  ...
  methods: {
    save() {
      ...
      this.request
        .send('post', '/products', this.product)
        .then(response => {
...
<template>
    <div>
        <button class="btn btn-success" @click="createProduct = true"><i class="fa fa-plus fa-first" aria-hidden="true"></i> {{$t('product.AddProduct')}}</button>
        <candy-modal title="Create Product" v-show="createProduct" size="modal-md" @closed="createProduct = false">
            <div slot="body">
                ...
                <div class="form-group">
                    <label for="redirectURL">{{$t('product.EnterSlugProduct')}}</label>
                    <input type="text" id="redirectURL" class="form-control" v-model="product.url">
                    <span class="text-info" v-if="product.url">{{$t('product.urlPrompt')}}: <code>{{ productUrl }}</code></span>
                    <span class="text-danger" v-if="request.getError('url')" v-text="request.getError('url')"></span>
                </div>


                <div class="form-group">
                    <label for="redirectURL">{{$t('product.ProductFamily')}}</label>
            ...
            <template slot="footer">
                <button type="button" class="btn btn-primary" @click="save">{{$t('product.CreateProduct')}}</button>
            </template>
        </candy-modal>
    </div>
</template>
