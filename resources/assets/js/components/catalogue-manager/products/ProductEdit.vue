      ...
      apiRequest
        .send(
          ...
          {},
          {
            excl_tax: true,
            includes:
              'family,variants.pricing.tax,variants.pricing.group,variants.tiers.group,variants.tax,assets,assets.tags,attribute_groups,attribute_groups.attributes,' +
        ...
        .then(response => {
          ...
          document.title =
            this.$options.filters.attribute(this.product, 'name') +
            ' Product - GetCandy';
        })
        .catch(error => {
    ...
    <div>
        <template v-if="loaded">
            <div class="alert alert-warning" v-if="!purchasable"><i class="fa fa-exclamation-triangle"
                                                                    aria-hidden="true"></i>
                {{$t('product.CannotPurchased')}} - <a href="#">{{$t('product.ClickFix')}}</a></div>
            <div class="alert alert-danger" v-if="!viewable">
                <i class="fa fa-exclamation-triangle" aria-hidden="true"></i> {{$t('product.CannotViewed')}} -
                <a href="#">{{$t('product.ClickFix')}}</a>
            </div>

            <transition name="fade">
                <candy-tabs initial="productdetails">

                    <candy-tab :name="$t('product.ProductDetails')" :selected="true" dispatch="product-details">
                    ...
                    <candy-tab :name="$t('product.Media')" dispatch="save-media">
                        <candy-media
                            assetable="products"
                            :parent="product">
                        </candy-media>
                    ...
                    <candy-tab
                        :name="$t('product.AvailabilityPricing')"
                    ...
                    <candy-tab :name="$t('product.Associations')">
                        <candy-tabs nested="true">
                            <candy-tab :name="$t('product.Categories')" handle="categories" :selected="true" :badge="getCategoryCount()">
                                <candy-categories :product="product"></candy-categories>
                            </candy-tab>
                            <candy-tab :name="$t('product.Collections')" handle="collections">
                                <candy-product-collections :product-id="product.id" :existing="product.collections.data"></candy-product-collections>
                            </candy-tab>
                            <candy-tab :name="$t('product.Products')" handle="products" :badge="getAssociationCount()" dispatch="product-associations">
                                <candy-products :product="product"></candy-products>
                            </candy-tab>
                        </candy-tabs>
                    </candy-tab>

                    <candy-tab :name="$t('product.Display')">
                        <candy-display></candy-display>
                    </candy-tab>

                    <candy-tab :name="$t('product.URLS')" dispatch="save-urls">
                        <candy-tabs nested="true">
                            <candy-tab :name="$t('product.LocaleURLS')" handle="locale-urls" :selected="true" dispatch="save-urls">
                                <candy-urls :languages="languages" :routes="routes" :model="product" endpoint="products"></candy-urls>
                            </candy-tab>
                            <candy-tab :name="$t('product.Redirects')" handle="redirects" dispatch="product-details">
