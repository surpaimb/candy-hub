...
export default {
  ...
  mounted() {
    ...
    CandyEvent.$on('asset_deleted', event => {
      ...
      this.variants.forEach(variant => {
        if (
          this.hasThumbnail(variant) &&
          variant.thumbnail.data.id == event.asset.id
        ) {
  ...
  methods: {
    ...
    save() {
      ...
      this.request
        .send('put', '/products/variants/' + this.current.id, data)
        .then(response => {
                    ...
                    <div class="col-xs-12"
                                    ...
                                    <div class="form-inline">
                                        <div class="form-group">
                                            <div v-show="translating">
                                                <label class="sr-only">{{$t('product.Language')}}</label>
                                                <candy-select :options="languages" v-model="translationLanguage" v-if="languages.length"></candy-select>
                                            </div>
                                        </div>
                                        <button v-if="!translating" class="btn btn-default" @click="translating = true">{{$t('product.Translate')}}</button>
                                        <button v-if="translating" class="btn btn-default" @click="translating = false">{{$t('product.HideTranslation')}}</button>
                                    </div>
                                </div>
                            </div>
                            <hr>
                                ...
                                <div class="col-xs-12 col-md-3">
                                    ...
                                    <button class="variant-option-img" @click="changeImage = true">
                                        ...
                                        <span class="change-img">
                                            <span v-if="hasThumbnail(current)">{{$t('product.ChangeImage')}}</span>
                                            <span v-else>{{$t('product.ChooseImage')}}</span>
                                        </span>
                                    </button>

                                    <candy-modal :title="$t('product.ChangeVariantImage')" v-show="changeImage"
                        ...
                        <div class="row">
                            <div class="col-xs-12 col-md-4">
                                <div class="form-group">
                                    <label>
                                        {{$t('product.UnitQuantity')}}
                                        <em class="help-txt">{{$t('product.NumberOfUnitsPrice')}}</em>
                                    </label>
                            ...
                            <div class="col-xs-12 col-md-4">
                                <div class="form-group">
                                    <label>
                                        {{$t('product.MinPurchaseQuantity')}}
                                        <em class="help-txt">{{$t('product.MinimumAmountPurchased')}}</em>
                                    </label>
                            ...
                            <div class="col-xs-12 col-md-4">
                                <div class="form-group">
                                    <label>
                                        {{$t('product.MaxPurchaseQuantity')}}
                                        <em class="help-txt">{{$t('product.MaximumAmountPurchased')}}</em>
                                    </label>
                            ...
                            <div class="col-xs-12 col-md-12">
                                <div class="form-group">
                                    <label for="groupPricing">
                                        ...
                                        <span class="faux-label"> {{$t('product.IndividualCustomerGroupPricing')}}</span>
                                    </label>
                                </div>
                        ...
                        <div class="row" v-for="(tier, index) in priceTiers" v-if="customerGroupSelect.length">
                            ...
                            <div class="col-md-6">
                                <label>{{$t('product.CustomerGroup')}}</label>
                                <div class="row">
                                    <div class="col-md-10">
                        ...
                        <div class="form-group">
                            <button class="btn btn-primary" @click="addPriceTier">
                                <fa icon="plus"></fa> {{$t('product.AddTier')}}
                            </button>
                        </div>
                                ...
                                <div class="form-group">
                                    ...
                                    <em class="text-info help-txt">
                                            <span v-if="current.backorder == 'in-stock'">
                                                {{$t('product.CannotBoughtInStock')}}
                                            </span>
                                            <span v-if="current.backorder == 'expected'">
                                                {{$t('product.CannotBoughtExpected')}}
                                            </span>
                                            <span v-if="current.backorder == 'always'">
                                                {{$t('product.CannotBoughtAlways')}}
                                            </span>
                                        </em>
                                </div>
                            </div>
                        ...
                        <div class="row">
                            <div class="col-md-12">
                                <button class="btn btn-danger" @click="deleteVariant(currentIndex)"
                                    v-if="variants.length > 1"><i class="fa fa-trash"></i> {{$t('product.DeleteVariant')}}
                                </button>
                            </div>
                       </div>
                    </div>
