...
export default {
  ...
  mounted() {
    ...
    this.options.forEach(option => {
      this.$set(this.variant.options, option.handle, {
        [locale.current()]: '',
      });
    });
  },
...
<template>
    <div>
        <button class="btn btn-primary" @click="modalOpen = true">{{$t('product.CreateVariant')}}</button>
        <candy-modal :title="$t('product.CreateVariant')" v-show="modalOpen" @closed="modalOpen = false">
            <div slot="body" class="text-left">
                ...
                <div class="row">
                    ...
                    <div class="col-md-4">
                        <label for="sku">{{$t('product.Inventory')}}</label>
            ...
            <template slot="footer">
                <button class="btn btn-primary" @click="save()">{{$t('product.SaveVariant')}}</button>
            </template>
        </candy-modal>
    </div>
</template>
