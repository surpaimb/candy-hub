...
<template>
    <div>
        <button class="btn btn-success" @click="createCollection = true"><i class="fa fa-plus fa-first" aria-hidden="true"></i> {{$t('collection.AddCollection')}}</button>
        <candy-modal :title="$t('collection.CreateCollection')" v-show="createCollection" size="modal-md" @closed="createCollection = false">
            <div slot="body">
                <div class="form-group">
                    <label for="name">{{$t('collection.CollectionName')}}</label>
                ...
                <div class="form-group">
                    <label for="redirectURL">{{$t('collection.CollectionURL')}}</label>
                    <input type="text" id="redirectURL" class="form-control" v-model="collection.url">
                    <span class="text-info" v-if="collection.url">{{$t('collection.urlPrompt')}}: <code>{{ url }}</code></span>
                    <span class="text-danger" v-if="request.getError('url')" v-text="request.getError('url')"></span>
                </div>
            </div>
            <template slot="footer">
                <button type="button" class="btn btn-primary" @click="save">{{$t('collection.CreateCollection')}}</button>
            </template>
        </candy-modal>
    </div>
</template>
